<script>
  ...
  function hapus(id, judul)
  {
    ...
    .then((result) => {
      if (result.value) {
        $.ajax({
          url         : '<?=site_url('admin/kisah/destroy');?>',
          method      : 'POST',
          data        : { 
            id:id, 
            judul:judul, 
        ...
        .done(function(result){
          ...
          if(result.code == 400){
            ...
            setTimeout(function(){
              ...
              $.unblockUI();
              window.location.replace('<?=site_url('admin/kisah/index');?>');
            }, 2000);
  ...
  function verify(id, judul, status)
  {
    swal.fire({
      type: "question",
      title: "Terbitkan Kisah?",
      html: "Kamu akan menerbitkan kisah "+judul+"?",
      focusConfirm: false,
      showConfirmButton: true,
      showCancelButton: true,
      allowOutsideClick: false,
    ...
    .then((result) => {
      if (result.value) {
        $.ajax({
          url         : '<?=site_url('admin/kisah/verify');?>',
          method      : 'POST',
          data        : { 
            id:id, 
            judul:judul, 
